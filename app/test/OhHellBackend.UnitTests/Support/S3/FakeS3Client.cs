...
namespace OhHellBackend.UnitTests.Support.S3
{
    ...
    {
        private readonly Dictionary<string, PutObjectResponse> _configuredRequests;
        ...
        public FakeS3Client()
        {
            ...
            _configuredRequests = new Dictionary<string, PutObjectResponse>();
        }
        public void ConfigureSuccessfulPutRequest(string bucketName)
        {
            _configuredRequests[bucketName] = new PutObjectResponse
            {
                HttpStatusCode = HttpStatusCode.OK,
            };
        }

        public void ConfigureFailingPutRequest(string bucketName)
        {
            _configuredRequests[bucketName] = new PutObjectResponse
            {
        ...
        public void ConfigurePutRequest(string bucketName, PutObjectResponse response)
        {
            _configuredRequests[bucketName] = response;
        }
        ...
        )
        {
            ...
            if (_configuredRequests.ContainsKey(request.BucketName))
            {
                return await Task.FromResult(_configuredRequests[request.BucketName]);
            }
            return await Task.FromResult(new PutObjectResponse { HttpStatusCode = HttpStatusCode.NotFound });
        }
